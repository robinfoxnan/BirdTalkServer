...
但是后期发现了一个冲突，就是按用户存储（pk, uid1, id）的时候，无法快速过滤针对某个会话的消息，这样在客户端拖动界面加载的时候并不方便，所有使用了物化视图，（pk, uid1, uid2, id）这样可以快速查找，但是这样等于一份消息存储了4遍；



对比TINODE使用针对会话的方式存储，确实效率比较低。如果使用会话方式存储的问题在于登录后，需要遍历所有可能的会话来加载数据；而且如果使用临时收件箱也不现实，因为考虑多终端同步，也不可能做到收完消息就清理临时收件箱，最后还是会退化到2种存储方式。
