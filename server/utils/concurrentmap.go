...
type ClearCb[V any] func(old V, client V) bool

// 带回调的删除，根据客户给的参数值，在遍历时候由回调函数决定是否需要删除
func (m ConcurrentMap[K, V]) clearWithCb(client V, cb ClearCb[V]) {

	for item := range m.IterBuffered() {
		b := cb(item.Val, client)
		if b {
			m.Remove(item.Key)
		}
