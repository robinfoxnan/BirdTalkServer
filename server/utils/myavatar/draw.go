...
func Draw(size int, letter string, options *Options) (image.Image, error) {
	font := defaultFont
...
func drawAvatar(bgColor, fgColor color.Color, font *truetype.Font, size int, letter string) (image.Image, error) {
	dst := newRGBA(size, size, bgColor)

	fontSize := float64(size) * 0.35
